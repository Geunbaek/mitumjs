...
class Currency extends types_1.Generator {
    ...
    async getAllCurrencies() {
        const datas = await (0, api_1.getAPIData)(() => api_1.default.currency.getCurrencies(this.api));
        return datas
            ? Object.keys(datas._links).filter(c => !(c === "self" || c === "currency:{currencyid}")).map(c => c)
            : null;
    }
    async getCurrency(cid) {
        const data = await (0, api_1.getAPIData)(() => api_1.default.currency.getCurrency(this.api, cid));
...
class Account extends key_1.KeyG {
    ...
    createWallet(sender, currency, amount, seed, weight) {
        ...
        const ks = new key_1.Keys([new key_1.PubKey(kp.publicKey, weight ?? 100)], weight ?? 100);
        return {
            wallet: {
    ...
    createEtherWallet(sender, currency, amount, seed, weight) {
        ...
        const ks = new key_1.EtherKeys([new key_1.PubKey(kp.publicKey, weight ?? 100)], weight ?? 100);
        return {
            wallet: {
                privatekey: kp.privateKey.toString(),
                publickey: kp.publicKey.toString(),
    ...
    async touch(privatekey, wallet) {
        ...
        op.sign(privatekey);
        return await (0, api_1.getAPIData)(() => api_1.default.operation.send(this.api, op.toHintedObject()));
    }
    async getAccountInfo(address) {
        const data = await (0, api_1.getAPIData)(() => api_1.default.account.getAccount(this.api, address));
        return data ? data._embedded : null;
    }
    async getOperations(address) {
        const data = await (0, api_1.getAPIData)(() => api_1.default.operation.getAccountOperations(this.api, address));
        return data ? data._embedded : null;
    }
    async getByPublickey(publickey) {
        const data = await (0, api_1.getAPIData)(() => api_1.default.account.getAccountByPublicKey(this.api, publickey));
        return data ? data._embedded : null;
    }
